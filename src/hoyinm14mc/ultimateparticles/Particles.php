...
class Particles {
	 ...
	 * @return bool
	 */
	public function showParticleEffects(Player $player, Vector3 $from) : bool {
		...
		foreach ($t[$player->getName()]["particles"] as $particle => $value) {
			if ($t[$player->getName()]["particles"][$particle]["type"] === "particle") {
				for ($i = 0; $i < (int)$t[$player->getName()]["particles"][$particle]["amplifier"]; $i++) {
					if ($t[$player->getName()]["particles"][$particle]["shape"] === "trail") {
	 ...
	 * @return Particle
	 */
	public function getParticle(Vector3 $vector3, string $particle, int $data = null) {
		switch (strtolower($particle)) {
			...
			case "redstone":
				return new RedstoneParticle($vector3, (!isset($data) ? 1 : $data));
			case "snowball":
		...
		if (substr($particle, 0, 6) == "block_") {
			return new TerrainParticle($vector3, Block::get((int)substr($particle, 7)));
		}
		//dust_rgb1,rgb2,rgb3
		if (substr($particle, 0, 5) == "dust_") {
			$rgb = explode(substr($particle, 6), ",");
			if ($rgb[0] > -1 and $rgb[0] < 256 and $rgb[1] > -1 and $rgb[1] < 256 and $rgb[2] > -1 and $rgb[2] < 256) {
